...
module Year2016.Day09 () where
import RIO.Text as T
import RIO
import RIO.List as L
import Prelude (head, last, read, repeat)
...
expand m s =
  let marker = T.split (== 'x') m
      chars = (read . T.unpack $ head marker :: Int)
      n = (read . T.unpack . last $ marker :: Int)
      expandee = T.take chars s
      expansion =  T.concat $ L.take n $ repeat expandee
  in expansion <> T.drop chars s

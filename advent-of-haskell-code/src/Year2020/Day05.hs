...
parseBoardingPass pass =
  let
    (rowInstructions, columnInstructions) = (T.take 7 pass, T.drop 7 pass)
  in
    (binarySplit rowInstructions [0..127], binarySplit columnInstructions [0..7])

binarySplit :: Text -> [Int] -> Int
binarySplit "" rows = head rows
binarySplit instructions rows =
  let
    (lowerHalf, upperHalf) = splitInHalf rows
  in
    if (T.head instructions `elem` ['F', 'L'])

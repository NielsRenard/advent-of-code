  ...
  let uniqueAnswersPerGroup =
        L.map (S.fromList . T.unpack) $
          L.map (T.replace " " "") $
            L.map T.unwords $
              splitWhen (== "") input
   in L.foldr ((+) . L.length) 0 uniqueAnswersPerGroup

solvePart2 input =
  let answersPerGroup :: [[Text]] = splitWhen (== "") input
      listOfListsOfSetsOfAnswers :: [[S.Set Char]] = (L.map (L.map (S.fromList . T.unpack))) answersPerGroup
   in foldr (+) 0 $
        L.map L.length $
          L.map
            (L.foldl1 S.intersection)
            listOfListsOfSetsOfAnswers
